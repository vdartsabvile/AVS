...
class PreLoginWindow {
    ...
    fun openLoginForm(owner: Stage, onLoginSuccess: (Boolean) -> Unit) {
        ...
        val registerButton = Button("Регистрация")
        registerButton.setOnAction {
            RegistrationWindow().openRegistrationForm(loginStage)
            println("Открытие окна регистрации")
        }
        ...
        val layout = VBox(
            ...
            loginButton,
            Label(),
            Label("Нет аккаунта? Зарегестрируйтесь!"),

        )
        val layout2 = HBox(registerButton,
            Label(" или "),
            cancelButton)
        layout.alignment = Pos.CENTER
        layout.setStyle("-fx-padding: 20;")
        layout2.alignment = Pos.CENTER
