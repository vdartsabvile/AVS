...
class ClientFriendProfileWindow (private val friend: Friend) {
    ...
    fun openClientFriendProfileWindow(primaryStage: Stage) {
        val root = BorderPane()
        ...
        val layout = VBox(10.0, chatArea, messageField, sendButton)
    ...
    private fun sendMessage() {
        val recipient = friend.friendUsername
        val sender = AppState.username
        ...
        if (recipient.isNotEmpty() && content.isNotEmpty()) {
            ...
            webSocket.send(message.toString()) // Отправляем получателю

            // Создаём сообщение для себя, если необходимо
            val selfMessage = JSONObject().apply {
                put("type", "chat")
                put("recipient", sender) // Отправляем себе
                put("content", content)
            }

            webSocket.send(selfMessage.toString()) // Отправляем себе
    ...
    private fun handleCallAccepted(json: JSONObject) {
        val from = json.getString("from")
        Platform.runLater { appendToChatArea("$from accepted your call") }
    }

    private fun handleCallRejected(json: JSONObject) {
        val from = json.getString("from")
        Platform.runLater {
            appendToChatArea("$from rejected your call")
            stopPulsatingEffect(members)
    ...
    private fun animateButtonHover(button: Button, hover: Boolean) {
        val scaleTransition = ScaleTransition(Duration.millis(140.0), button)
    ...
    fun setupButtons(participants: HBox, primaryStage: Stage): HBox {
        ...
        callButton.setOnAction {
            ...
            if (isCalling) {
                ...
                stopPulsatingEffect(members)
                participants.children.clear()
                showErrorPopup("Звонок был завершен.", primaryStage)
